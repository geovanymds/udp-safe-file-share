  ...
  for(int i = 2; i < argc; i++) {
        ...
        while(receiving == 1){
          ...
          if(result == SOCKET_ERROR){
            ...
            if(pacote[BUFFER_SIZE-1] != 1){
              fwrite(&pacote, sizeof(unsigned char), (long int) pacote[BUFFER_SIZE-1], writer);
            }
            else{
              fwrite(&pacote, sizeof(unsigned char), BUFFER_SIZE-17, writer);
            }
            receiving = pacote[BUFFER_SIZE-1];
          }
        }
        fclose(writer);
        printf("FINE!!!!");
